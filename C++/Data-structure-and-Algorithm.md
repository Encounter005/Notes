...
1. 在二叉树的的第$i$层上至多有$2^{i - 1}$个节点

2. 深度为$k$的二叉树至多有$2^{k - 1}$个节点

3. 终端节点数为$n$,度为2的节点数为$n_{2}$，则$n = n_{2} + 1$

4. 给定一棵树，可以找到唯一的一棵二叉树与之对应。

5. 若想计算某种节点的数量，需要节点总数、位置、连接方式


完全二叉树

1. 在完全二叉树中，具有$n$个节点的完全二叉树的深度为$[\log_{2}{n}] + 1$

2. 一颗完全二叉树，其叶子节点的个数是确定的

3. 

满二叉树的性质

1. 节点总数 = $2^k - 1$ $k$为层数 , 叶子节点数 = $2^{k - 1}$
...
1. 在n个节点的树中有n-1条边
2. 给任意多种度数，求节点数 $n_{0} = 1 + n_{2} + 2n_{3} + ... + (m - 1)n_{m}$
